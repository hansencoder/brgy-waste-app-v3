            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2.5 shrink-0 flex-wrap">
                        <button onclick="window.print()" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white hover:bg-slate-100 text-slate-700 font-extrabold text-xs sm:text-sm border border-slate-200 shadow-2xs transition active:scale-[0.98] cursor-pointer" title="Print Audit Report">
                        ...
                        <a href="/brgy-waste-app-v3/public/admin/exportAuditLogs" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#0B2E22] hover:bg-[#084232] text-white font-extrabold text-xs sm:text-sm shadow-xs transition active:scale-[0.98] border border-emerald-900">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-[#10B981]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
...
<div id="printArea" class="hidden">
    <div class="p-8 space-y-6">
        ...
        <div class="flex items-center justify-between border-b-2 border-slate-800 pb-4">
            <div class="w-16 h-16">
                <img src="<?php echo htmlspecialchars($sysLogo); ?>" class="h-16 w-16 object-contain">
            </div>
        ...
        <table class="w-full text-xs text-left border-collapse border border-slate-300">
            <thead>
                <tr class="bg-slate-100">
                    <th class="border border-slate-300 p-2 font-bold">Timestamp</th>
                    <th class="border border-slate-300 p-2 font-bold">User</th>
                    <th class="border border-slate-300 p-2 font-bold">Action</th>
                    <th class="border border-slate-300 p-2 font-bold">Target</th>
                    <th class="border border-slate-300 p-2 font-bold">Details</th>
                    <th class="border border-slate-300 p-2 font-bold">Result</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach (array_slice($logs, 0, 100) as $l): ?>
                    <tr>
                        <td class="border border-slate-300 p-2 font-mono whitespace-nowrap"><?php echo date('m/d/Y h:i A', strtotime($l['created_at'])); ?></td>
                        <td class="border border-slate-300 p-2 font-bold"><?php echo htmlspecialchars($l['user_name'] ?? 'System'); ?></td>
                        <td class="border border-slate-300 p-2"><?php echo htmlspecialchars($l['action']); ?></td>
                        <td class="border border-slate-300 p-2 font-mono"><?php echo htmlspecialchars($l['affected_record'] ?? 'N/A'); ?></td>
                        <td class="border border-slate-300 p-2"><?php echo htmlspecialchars($l['details'] ?? ''); ?></td>
                        <td class="border border-slate-300 p-2 font-bold uppercase"><?php echo htmlspecialchars($l['result'] ?? 'SUCCESS'); ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
