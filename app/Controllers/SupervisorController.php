...
/**
 * Export analytics as CSV (Excel-compatible)
 */
public function exportAnalyticsExcel() {
    ...
    $db->query("
        SELECT r.id, r.submission_date, u.name as reporter,
               rs.status_name as status, wc.category_name as category,
               p.purok_name as purok, r.support_count
        FROM reports r
        JOIN users u ON r.resident_id = u.id
    ...
    fputcsv($output, ['Generated By', $_SESSION['user_name'] ?? 'Supervisor']);
    fputcsv($output, []);
    fputcsv($output, ['KPI Summary']);
    fputcsv($output, ['Total Reports', $stats['total'] ?? 0]);
    fputcsv($output, ['Pending', $stats['pending'] ?? 0]);
    fputcsv($output, ['Verified', $stats['verified'] ?? 0]);
    fputcsv($output, ['In Progress', $stats['in_progress'] ?? 0]);
    fputcsv($output, ['Resolved', $stats['resolved'] ?? 0]);
    fputcsv($output, []);
    fputcsv($output, ['Report ID', 'Date', 'Reporter', 'Category', 'Purok', 'Status', 'Supports']);
